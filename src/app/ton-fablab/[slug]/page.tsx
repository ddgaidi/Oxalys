...
export default function FabLabDetailPage() {
  const params = useParams<{ slug?: string | string[] }>();
  const slug = Array.isArray(params?.slug) ? params.slug[0] : params?.slug;
  const [initialFabLab, setInitialFabLab] = useState<FabLab | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadFabLab() {
      if (!slug) {
        setInitialFabLab(null);
        setMissing(true);
        setLoading(false);
        return;
      }
      ...
      const fablab = await fetchFabLabById(slug);
      if (cancelled) return;

      setInitialFabLab(fablab);
      setMissing(!fablab);
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <FabLabDetailFallback
        isDark={isDark}
        title="Chargement du FabLab..."
        description="Récupération des informations et du statut capteur."
      />
    );
  }

  if (missing || !initialFabLab) {
    return (
      <FabLabDetailFallback
        isDark={isDark}
        title="FabLab introuvable"
        description="Ce FabLab n'existe pas ou n'est plus disponible."
        showBackLink
      />
    );
  }

  return <FabLabDetailContent key={initialFabLab.id} fablab={initialFabLab} />;
}

function FabLabDetailFallback({
  isDark,
  title,
  description,
  showBackLink = false,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-5" style={{ background: pageBg }}>
      <div
        className="max-w-md w-full text-center rounded-2xl p-8"
        style={{ background: cardBg, border: `1px solid ${cardBorder}` }}
      >
        <Wrench size={36} className="mx-auto mb-4 text-blue-500" />
        <h1 className="font-display font-bold text-xl mb-2" style={{ color: titleColor }}>
          {title}
        </h1>
        ...
        {showBackLink && (
          <Link href="/ton-fablab" className="btn-primary inline-flex mt-6 px-5 py-2.5 text-sm rounded-xl">
            Retour aux FabLabs
          </Link>
        )}
      </div>
...
function FabLabDetailContent({ fablab: initialFabLab }: { fablab: FabLab }) {
...
// Composant principal : orchestre les donnees, le theme et le rendu de cette vue.
function FabLabFavoriteButton({ fablabId, isDark }: Props) {
